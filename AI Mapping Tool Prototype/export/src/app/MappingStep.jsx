...
function MappingStep({ file, onConfirm, onBack }) {
  ...
  const columnProfile = useMemoM(() => {
    ...
    for (const h of file.headers) {
      ...
      for (let i = 0; i < scan; i++) {
        ...
        if (v) { filled++; if (seen.size < 1000) seen.add(v); }
      }
      prof[h] = { distinct: seen.size, fillPct: scan ? Math.round((filled / scan) * 100) : 0 };
    }
    return prof;
  }, [file]);
    ...
    (async () => {
      ...
      try {
        ...
        if (saved) {
          ...
          if (!cancelled) {
            setSavedApplied({ key: formatKey, label: saved.label, savedAt: saved.savedAt });
  ...
  const setTarget = (header, value) => {
    setMappings(curr => curr.map(m => {
      ...
      const keep = { fill_rate: m.fill_rate, data_profile: m.data_profile, recommended_from_data: false };
      if (value === "") {
        return { ...m, ...keep, target_field: null, custom_field_name: null, status: "unmapped", manual: true, confidence: 0 };
      }
      if (value === CUSTOM_VALUE) {
        const samples = (file.rows || []).slice(0, 20).map(r => r[header]).filter(Boolean);
        return {
          ...m, ...keep, target_field: CUSTOM_VALUE,
      ...
      return { ...m, ...keep, target_field: value, custom_field_name: null, status: "auto_confirmed", manual: true, confidence: 1 };
    }));
  };
  ...
  const setFdmType = (header, type) => {
    setMappings(curr => curr.map(m =>
      m.user_header === header ? { ...m, fdm_type: type } : m
    ));
  };
  ...
  const setOptionRoute = (header, optName, target) => {
    setMappings(curr => curr.map(m => {
      ...
      const routes = { ...m.option_routes };
      const T = window.Transforms;
      if (target === "") routes[optName] = { target: null, reason: "skipped" };
      else if (target === CUSTOM_VALUE) routes[optName] = { target: "__custom__", custom: T.titleCase(optName), reason: "custom field" };
      else routes[optName] = { target, reason: "manual" };
      return { ...m, option_routes: routes, manual: true };
    }));
  };
  ...
  const toggleSplit = (header) => {
    setMappings(curr => curr.map(m => {
      if (m.user_header !== header) return m;
      if (m.transform && m.transform.type === "split") return { ...m, transform: null, manual: true };
      // Re-detect a delimiter from the data
      const samples = (file.rows || []).slice(0, 40).map(r => r[header]).filter(Boolean);
      const mv = window.Transforms.detectMultiValue(samples) || { delimiter: ",", label: "comma", avgCount: 2 };
      return { ...m, transform: { type: "split", ...mv }, manual: true };
    }));
  };
  ...
  const toggleStripHtml = (header) => {
    setMappings(curr => curr.map(m => {
      if (m.user_header !== header) return m;
      if (m.transform && m.transform.type === "strip_html") return { ...m, transform: null, manual: true };
      return { ...m, transform: { type: "strip_html", label: "strip HTML" }, manual: true };
    }));
  };
  ...
  const acceptAllRecommendations = () => {
    setMappings(curr => curr.map(m => {
      if ((m.status === "recommended" || m.status === "low_confidence") && m.target_field && m.target_field !== CUSTOM_VALUE) {
        return { ...m, status: "auto_confirmed", manual: true, recommended_from_data: false, reason: "Accepted recommendation" };
      }
      return m;
    }));
  };
  const skipAllUnmapped = () => {
    setMappings(curr => curr.map(m =>
      (m.status === "unmapped")
        ? { ...m, target_field: null, custom_field_name: null, manual: true }
        : m
    ));
  };
  const keepAllCustom = () => {
    setMappings(curr => curr.map(m =>
      (m.status === "low_confidence")
        ? { ...m, target_field: CUSTOM_VALUE, custom_field_name: m.custom_field_name || m.user_header, status: "custom", manual: true }
        : m
    ));
  };
  ...
  const forgetSaved = () => {
    if (savedApplied) { window.SavedMappings.forget(savedApplied.key); setSavedApplied(null); }
  };
  ...
  const requiredUnmapped = useMemoM(() => {
    ...
    return window.TARGET_SCHEMA.filter(f =>
      f.required
      && !satisfied(f.id)
      && !window.ACCOUNT_PROVIDES.includes(f.id)   // brand_name etc. come from the logged-in account
  ...
  useEffectM(() => {
    ...
    if (Object.keys(seed).length) setConstants(c => ({ ...seed, ...c }));
  ...
  const accountSatisfied = useMemoM(() => {
    ...
    return window.TARGET_SCHEMA.filter(f =>
      f.required
      && window.ACCOUNT_PROVIDES.includes(f.id)
      && !mapped.has(f.id)
    );
  }, [mappings]);

  const counts = useMemoM(() => {
    if (!mappings) return { auto: 0, rec: 0, low: 0, un: 0, custom: 0, routed: 0, system: 0, split: 0 };
    return {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 32px 24px", width: "100%" }}>
        ...
        {/* Multi-row structure summary (PRD Gap #1) */}
        {file.structure && file.structure.detected && (
          <div style={{
            display: "flex", alignItems: "flex-start", gap: 14,
            padding: "14px 18px", marginBottom: 16, borderRadius: 3,
            background: "rgba(65,175,75,0.06)", border: "1px solid rgba(65,175,75,0.28)",
          }}>
            <div style={{
              width: 32, height: 32, borderRadius: 3, flexShrink: 0,
              background: "var(--hl-do-green)", color: "#fff",
              display: "inline-flex", alignItems: "center", justifyContent: "center",
            }}>
              <i className="fas fa-sitemap"/>
            </div>
            <div style={{ flex: 1, fontSize: 13, lineHeight: "19px", color: "var(--hl-fg-1)" }}>
              <div style={{ fontWeight: 700, color: "var(--hl-icon)", marginBottom: 2 }}>
                Multi-row file detected — grouped by <code style={{ fontFamily: "var(--hl-font-mono)", background: "var(--hl-framing)", padding: "1px 5px", borderRadius: 2 }}>{file.structure.groupKey}</code>
              </div>
              <div style={{ color: "var(--hl-fg-2)" }}>
                We grouped <b>{file.structure.originalRowCount.toLocaleString()}</b> rows into{" "}
                <b>{file.structure.productCount.toLocaleString()}</b> product{file.structure.productCount === 1 ? "" : "s"}
                {file.structure.variantCount > 0 && <> + <b>{file.structure.variantCount.toLocaleString()}</b> variant{file.structure.variantCount === 1 ? "" : "s"}</>}
                {file.structure.imageRowCount > 0 && <>, set aside <b>{file.structure.imageRowCount.toLocaleString()}</b> additional-image row{file.structure.imageRowCount === 1 ? "" : "s"}</>}
                {file.structure.filledCells > 0 && <>, and copied shared product details down into <b>{file.structure.filledCells.toLocaleString()}</b> blank variant cell{file.structure.filledCells === 1 ? "" : "s"}</>}.
              </div>
            </div>
          </div>
        )}

        {/* Saved-mapping applied banner (#2) */}
        {savedApplied && (
          <div style={{
            display: "flex", alignItems: "center", gap: 14,
            padding: "12px 18px", marginBottom: 16, borderRadius: 3,
            background: "rgba(5,141,233,0.07)", border: "1px solid rgba(5,141,233,0.3)",
          }}>
            ...
            <div style={{ flex: 1, fontSize: 13, lineHeight: "18px", color: "var(--hl-fg-1)" }}>
              <span style={{ fontWeight: 700, color: "var(--hl-icon)" }}>Saved mapping applied — “{savedApplied.label}”.</span>{" "}
              We remembered this format from a previous upload and pre-filled every column. Review and continue, or
              <button onClick={forgetSaved} style={{ background: "none", border: 0, padding: "0 0 0 4px", color: "var(--hl-go-blue)", fontWeight: 700, cursor: "pointer", fontFamily: "inherit", fontSize: 13 }}>start fresh</button>.
            </div>
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
          {counts.auto    > 0 && <FilterChip tone="success" icon="check"              label={`${counts.auto} auto-confirmed`}    active={filter === "auto_confirmed"} onClick={() => setFilter(filter === "auto_confirmed" ? null : "auto_confirmed")}/>}
          {counts.rec     > 0 && <FilterChip tone="warn"    icon="exclamation-circle" label={`${counts.rec} need review`}        active={filter === "recommended"}    onClick={() => setFilter(filter === "recommended"    ? null : "recommended")}/>}
          {counts.low     > 0 && <FilterChip tone="warn"    icon="search"             label={`${counts.low} low confidence`}     active={filter === "low_confidence"} onClick={() => setFilter(filter === "low_confidence" ? null : "low_confidence")}/>}
          {counts.custom  > 0 && <FilterChip tone="info"    icon="plus-circle"        label={`${counts.custom} custom field${counts.custom === 1 ? "" : "s"}`} active={filter === "custom"} onClick={() => setFilter(filter === "custom" ? null : "custom")}/>}
          {counts.routed  > 0 && <FilterChip tone="violet"  icon="code-branch"        label={`${counts.routed} conditional`} active={filter === "option_route"} onClick={() => setFilter(filter === "option_route" ? null : "option_route")}/>}
          {counts.split   > 0 && <FilterChip tone="violet"  icon="scissors"           label={`${counts.split} multi-value`} active={filter === "__split__"} onClick={() => setFilter(filter === "__split__" ? null : "__split__")}/>}
          {counts.un      > 0 && <FilterChip tone="err"     icon="minus-circle"       label={`${counts.un} skipped`}             active={filter === "unmapped"}       onClick={() => setFilter(filter === "unmapped"       ? null : "unmapped")}/>}
          {filter && (
            <button onClick={() => setFilter(null)}
              style={{
                marginLeft: 4, height: 28, padding: "0 10px",
                background: "transparent", border: 0, cursor: "pointer",
                color: "var(--hl-go-blue)", fontFamily: "inherit", fontWeight: 700,
                fontSize: 13, display: "inline-flex", alignItems: "center", gap: 6,
              }}>
              <i className="fas fa-times" style={{ fontSize: 11 }}/>Clear filter
            </button>
          )}
        </div>

        {/* Bulk actions + group-by-status (#3) */}
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
          <span style={{ fontSize: 12, fontWeight: 700, color: "var(--hl-fg-3)", textTransform: "uppercase", letterSpacing: ".04em", marginRight: 2 }}>Bulk</span>
          {(counts.rec + counts.low) > 0 && (
            <BulkBtn icon="check-double" onClick={acceptAllRecommendations}>Accept all recommendations</BulkBtn>
          )}
          {counts.low > 0 && (
            <BulkBtn icon="plus-circle" onClick={keepAllCustom}>Keep low-confidence as custom</BulkBtn>
          )}
          {counts.un > 0 && (
            <BulkBtn icon="minus-circle" onClick={skipAllUnmapped}>Skip all unmapped</BulkBtn>
          )}
          <div style={{ flex: 1 }}/>
          <BulkBtn icon="table-list" onClick={() => setShowPreview(true)}>Preview data</BulkBtn>
          <BulkBtn icon={grouped ? "list" : "layer-group"} onClick={() => setGrouped(g => !g)} active={grouped}>
            {grouped ? "Flat list" : "Group by status"}
          </BulkBtn>
        </div>

        {accountSatisfied.length > 0 && (
          <div style={{
            padding: "10px 16px", borderRadius: 3,
            background: "rgba(5,141,233,0.06)", border: "1px solid rgba(5,141,233,0.20)",
            color: "var(--hl-fg-1)",
            display: "flex", alignItems: "center", gap: 12, fontSize: 13, marginBottom: 16
          }}>
            <i className="fas fa-user-shield" style={{ color: "var(--hl-go-blue)" }}/>
            <div style={{ flex: 1 }}>
              <span style={{ fontWeight: 700 }}>{accountSatisfied.map(f => f.label).join(", ")}</span> {accountSatisfied.length === 1 ? "comes" : "come"} from your account <b>(Test Brand)</b> — no column mapping needed. Map a column above to override per row.
            </div>
          </div>
        )}
        ...
        {requiredUnmapped.length > 0 && (
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 14, marginBottom: 10 }}>
              <i className="fas fa-exclamation-triangle"/>
              <div>
                <span style={{ fontWeight: 700 }}>Required fields aren't mapped: </span>
                {requiredUnmapped.map(f => f.label).join(", ")}. Map a column below — or set a constant value to apply to every row.
              </div>
            </div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, paddingLeft: 26 }}>
              {requiredUnmapped.map(f => (
                <div key={f.id} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: "var(--hl-fg-2)" }}>{f.label} =</span>
                  <input
                    type="text"
                    ...
                    style={{
                      height: 30, width: 160, borderRadius: 3, border: "2px solid var(--hl-keyline)",
                      padding: "0 10px", fontFamily: "inherit", fontSize: 13, fontWeight: 700, color: "var(--hl-icon)",
                    }}/>
                </div>
              ))}
            </div>
        ...
        {Object.keys(constants).length > 0 && (
          <div style={{
            padding: "10px 16px", borderRadius: 3, marginBottom: 16,
            background: "rgba(65,175,75,0.06)", border: "1px solid rgba(65,175,75,0.25)",
            display: "flex", alignItems: "center", gap: 12, fontSize: 13, color: "var(--hl-fg-1)",
          }}>
            <i className="fas fa-thumbtack" style={{ color: "var(--hl-do-green)" }}/>
            <div style={{ flex: 1 }}>
              <b>Constant values</b> applied to every row:{" "}
              {Object.entries(constants).map(([k, v], i) => (
                <span key={k}>
                  {i > 0 ? ", " : ""}
                  <b style={{ color: "var(--hl-icon)" }}>{(window.TARGET_SCHEMA.find(f => f.id === k) || {}).label || k}</b> = “{v}”
                </span>
              ))}
            </div>
          </div>
        ...
        <div style={{ background: "#fff", border: "2px solid var(--hl-keyline)", borderRadius: 3, overflow: "hidden" }}>
          <div style={{
            ...
            <div>Your file{filter ? <span style={{ marginLeft: 8, fontWeight: 400, textTransform: "none", letterSpacing: 0, color: "var(--hl-fg-3)" }}>showing {visibleMappings.length} of {mappings.length}</span> : null}</div>
            <div/>
            <div>NuORDER field</div>
            <div style={{ textAlign: "right" }}>Confidence</div>
          </div>
          {visibleMappings.length === 0 ? (
            <div style={{ padding: "40px 20px", textAlign: "center", color: "var(--hl-fg-3)", fontSize: 13 }}>
            ...
            GROUP_ORDER.map(st => {
              ...
              return (
                <React.Fragment key={st}>
                  <div style={{
                    padding: "8px 20px", background: "var(--hl-bg-subtle, #F0F4F4)",
                    borderTop: "1px solid var(--hl-keyline)",
                    fontSize: 11, fontWeight: 900, letterSpacing: ".06em", textTransform: "uppercase",
                    color: tone.fg, display: "flex", alignItems: "center", gap: 8,
                  }}>
                    <i className={`fas fa-${tone.icon}`} style={{ fontSize: 11 }}/>{tone.label} · {rowsIn.length}
                  </div>
                  {rowsIn.map(renderRow)}
                </React.Fragment>
              );
            })
          ) : visibleMappings.map(renderRow)}
        </div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <Button variant="do" disabled={!canProceed} onClick={() => {
      ...
      {showPreview && <RowPreview file={file} profile={columnProfile} onClose={() => setShowPreview(false)}/>}
    </div>
  );
}

// Row-preview drawer — browse the file's first rows with per-column profile
function RowPreview({ file, profile, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        width: "min(960px, 92vw)", height: "100%", background: "#fff",
        display: "flex", flexDirection: "column", boxShadow: "var(--hl-shadow-modal)",
        animation: "hl-pop 200ms cubic-bezier(0.2,0,0,1)",
      }}>
        <div style={{ padding: "16px 24px", borderBottom: "1px solid var(--hl-keyline)", display: "flex", alignItems: "center", gap: 12 }}>
          <i className="fas fa-table-list" style={{ color: "var(--hl-go-blue)" }}/>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, fontSize: 15, color: "var(--hl-icon)" }}>Data preview</div>
            <div style={{ fontSize: 12, color: "var(--hl-fg-3)" }}>{file.headers.length} columns · {(file.rows || []).length.toLocaleString()} rows · showing first {rows.length}</div>
          </div>
          <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: "50%", border: "0", background: "var(--hl-framing)", cursor: "pointer", color: "var(--hl-fg-2)" }}>
        ...
        <div style={{ overflow: "auto", flex: 1 }}>
          <table style={{ borderCollapse: "collapse", fontSize: 12, width: "max-content", minWidth: "100%" }}>
            <thead>
              <tr>
                {cols.map(h => {
                  ...
                  return (
                    <th key={h} style={{
                      position: "sticky", top: 0, background: "var(--hl-framing)", textAlign: "left",
                      padding: "8px 12px", borderRight: "1px solid var(--hl-keyline)", borderBottom: "2px solid var(--hl-keyline)",
                      whiteSpace: "nowrap", verticalAlign: "top",
                    }}>
                      <div style={{ fontWeight: 700, color: "var(--hl-icon)" }}>{h}</div>
                      <div style={{ fontSize: 10, fontWeight: 400, color: "var(--hl-fg-3)", marginTop: 2 }}>
            ...
            <tbody>
              {rows.map((r, ri) => (
                <tr key={ri} style={{ background: ri % 2 ? "var(--hl-bg-subtle, #F7FAFA)" : "#fff" }}>
                  {cols.map(h => (
                    <td key={h} style={{
                      padding: "6px 12px", borderRight: "1px solid var(--hl-framing)", borderBottom: "1px solid var(--hl-framing)",
                      whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 240,
                      color: "var(--hl-fg-1)", fontFamily: "var(--hl-font-mono)",
                    }} title={String(r[h] ?? "")}>{String(r[h] ?? "").slice(0, 80)}</td>
                  ))}
                </tr>
              ))}
            </tbody>
...
const STATUS_TONES = {
  ...
  option_route:   { fg: "#7A4E92",                bg: "rgba(150,95,176,0.07)", icon: "code-branch",        label: "Conditional"    },
  system:         { fg: "var(--hl-fg-3)",         bg: "var(--hl-framing)",     icon: "cog",                label: "Routing input"  },
...
function BulkBtn({ icon, children, onClick, active }) {
  ...
  return (
    <button onClick={onClick}
      ...
      style={{
        ...
        height: 30, padding: "0 12px", borderRadius: 3, cursor: "pointer", fontFamily: "inherit",
      ...
      <i className={`fas fa-${icon}`} style={{ fontSize: 11 }}/>{children}
    </button>
  );
}
...
function MappingRow({ row, sample, usedTargets, isDupCustom, onTargetChange, onCustomNameChange, onOptionRoute, onToggleSplit, onToggleStripHtml, onFdmType }) {
    ...
    <div style={{
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          ...
          {typeof row.fill_rate === "number" && row.fill_rate > 0 && row.fill_rate < 0.5 && (
            <span title="Sparse column — variant- or image-level data in a multi-row export"
              style={{
                display: "inline-flex", alignItems: "center", gap: 4,
                padding: "1px 7px", borderRadius: 999, fontSize: 10, fontWeight: 700,
                background: "rgba(150,95,176,0.12)", color: "#7A4E92",
              }}>
              <i className="fas fa-layer-group" style={{ fontSize: 8 }}/>
              variant · {Math.round(row.fill_rate * 100)}% filled
            </span>
          )}
          {row.tier_group && (
            <span title="Part of a tiered-price family"
              style={{
                display: "inline-flex", alignItems: "center", gap: 4,
                padding: "1px 7px", borderRadius: 999, fontSize: 10, fontWeight: 700,
                background: "rgba(5,141,233,0.12)", color: "var(--hl-go-blue)",
              }}>
              <i className="fas fa-layer-group" style={{ fontSize: 8 }}/>
              {row.tier_group} · tier {row.tier_index}
            </span>
          )}
        </div>
        {sample.length > 0 && (
        ...
        {row.data_profile && (
          <div style={{ marginTop: 4, marginLeft: 22, fontSize: 11, color: "var(--hl-fg-3)", display: "inline-flex", alignItems: "center", gap: 5 }}>
            <i className="fas fa-eye" style={{ fontSize: 9 }}/>
            looks like: <b style={{ color: "var(--hl-fg-2)" }}>{row.data_profile}</b>
          </div>
        )}
      </div>
      ...
      <div>
        {isSystem ? (
          <div style={{
            ...
            height: 40, padding: "0 12px", borderRadius: 3,
            border: "2px dashed var(--hl-keyline)", background: "var(--hl-framing)",
            color: "var(--hl-fg-2)", fontSize: 13, fontWeight: 700,
          }}>
            <i className="fas fa-cog" style={{ color: "var(--hl-fg-3)" }}/>
            {row.system_role || "Used for routing"}
          </div>
        ) : isOption ? (
          <OptionRouteEditor row={row} usedTargets={usedTargets} onOptionRoute={onOptionRoute}/>
        ) : (
          <>
            <TargetSelect
            ...
            {isCustom && (
              <div style={{ marginTop: 8 }}>
                ...
                {(row.name_source === "data" || row.name_source === "cell") && !row.manual && (
                  <div style={{ marginTop: 4, fontSize: 11, color: "#7A4E92", display: "inline-flex", alignItems: "center", gap: 5, fontWeight: 700 }}>
                    <i className="fas fa-wand-magic-sparkles" style={{ fontSize: 9 }}/>
                    {row.name_source === "cell" ? "Named from a label inside the data" : "Named from the column data"}
                  </div>
                )}
                <FdmTypeSelect value={row.fdm_type || "string"} onChange={onFdmType}/>
              </div>
            )}
            ...
            {row.target_field && row.target_field !== CUSTOM_VALUE && row.target_field !== "product_description" && row.target_field !== "seo_description" && (
              <SplitControl transform={row.transform} onToggle={onToggleSplit}/>
            )}

            {/* HTML cleanup control (#7) — descriptions */}
            {(row.target_field === "product_description" || row.target_field === "seo_description") && (
              <StripHtmlControl transform={row.transform} onToggle={onToggleStripHtml}/>
            )}

            {row.reason && row.status !== "unmapped" && (
              row.recommended_from_data && !row.manual ? (
                <div style={{
                  marginTop: 6, fontSize: 11, lineHeight: "15px",
                  color: "#7A5300", background: "rgba(250,199,55,0.14)",
                  border: "1px solid rgba(180,122,0,0.25)", borderRadius: 3,
                  padding: "5px 8px", display: "flex", gap: 6, alignItems: "flex-start",
                }}>
                  <i className="fas fa-lightbulb" style={{ marginTop: 1, color: "#B47A00" }}/>
                  <span><b>AI recommendation from data:</b> {row.reason}</span>
                </div>
              ) : (
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 8 }}>
          {row.status === "custom" ? (
            ...
            <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12, fontWeight: 700, color: t.fg }}>
              <i className="fas fa-code-branch"/>Routed
            </span>
          ) : isSystem ? (
            <span style={{ fontSize: 12, fontWeight: 700, color: "var(--hl-fg-3)" }}>—</span>
          ) : (
            <>
...
function OptionRouteEditor({ row, usedTargets, onOptionRoute }) {
  const routes = row.option_routes || {};
  const names = Object.keys(routes);
  return (
    <div style={{ border: "2px solid #C9B3D6", borderRadius: 3, overflow: "hidden", background: "rgba(150,95,176,0.04)" }}>
      <div style={{ padding: "7px 10px", background: "rgba(150,95,176,0.12)", display: "flex", alignItems: "center", gap: 8, fontSize: 11, fontWeight: 700, color: "#7A4E92" }}>
        <i className="fas fa-code-branch"/>
        Routes by “{row.option_name_header}” — {names.length} value{names.length === 1 ? "" : "s"} found
      </div>
        ...
        {names.map(nm => {
          ...
          return (
            <div key={nm} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{
                minWidth: 92, maxWidth: 92, fontSize: 12, fontWeight: 700, color: "var(--hl-icon)",
                overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
              }} title={nm}>{nm}</span>
              <i className="fas fa-arrow-right" style={{ color: "var(--hl-fg-3)", fontSize: 10 }}/>
              <div style={{ position: "relative", flex: 1 }}>
                <select
                  ...
                  style={{
                    ...
                    border: "2px solid var(--hl-keyline)", padding: "0 28px 0 10px",
                    background: "#fff", fontFamily: "inherit", fontWeight: 700, fontSize: 12,
                    color: "var(--hl-icon)", appearance: "none", cursor: "pointer",
                  }}>
                  <option value="">— Skip —</option>
                  <option value={CUSTOM_VALUE}>+ Custom: {window.Transforms.titleCase(nm)}</option>
                  {window.TARGET_SCHEMA_GROUPS.map(g => (
                    <optgroup key={g.id} label={g.label}>
                      {g.fields.map(f => {
                        const used = usedTargets[f.id] && f.id !== val;
                        return <option key={f.id} value={f.id} disabled={used}>{f.label}{used ? " · used" : ""}</option>;
                      })}
                    </optgroup>
                  ))}
                </select>
                <i className="fas fa-chevron-down" style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", color: "var(--hl-fg-3)", pointerEvents: "none", fontSize: 10 }}/>
              </div>
            </div>
          );
        })}
...
// ─── Multi-value split control ─────────────────────────────────────────
function SplitControl({ transform, onToggle }) {
  ...
  return (
    <button
      ...
      <i className={`fas fa-${on ? "scissors" : "grip-lines"}`} style={{ fontSize: 10 }}/>
      {on
        ? `Split on ${transform.label} → ~${transform.avgCount} values`
...
function StripHtmlControl({ transform, onToggle }) {
  ...
  return (
    <button
      ...
      <i className={`fas fa-${on ? "broom" : "code"}`} style={{ fontSize: 10 }}/>
...
function FdmTypeSelect({ value, onChange }) {
  const types = (window.Transforms && window.Transforms.FDM_TYPES) || ["string", "number", "money", "date", "boolean", "multi-value"];
  const ICONS = { string: "font", number: "hashtag", money: "dollar-sign", date: "calendar", boolean: "toggle-on", "multi-value": "list" };
  return (
    <div style={{ marginTop: 6, display: "flex", alignItems: "center", gap: 6 }}>
      <span style={{ fontSize: 10, fontWeight: 700, color: "var(--hl-fg-3)", textTransform: "uppercase", letterSpacing: ".04em" }}>FDM type</span>
      <div style={{ position: "relative" }}>
        <select value={value} onChange={e => onChange(e.target.value)}
          style={{
            height: 26, padding: "0 24px 0 24px", borderRadius: 3,
            border: "1px solid var(--hl-go-blue)", background: "rgba(5,141,233,0.06)",
            fontFamily: "inherit", fontWeight: 700, fontSize: 11, color: "var(--hl-go-blue)",
            appearance: "none", cursor: "pointer",
          }}>
          {types.map(t => <option key={t} value={t}>{t}</option>)}
        </select>
        <i className={`fas fa-${ICONS[value] || "font"}`} style={{ position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)", color: "var(--hl-go-blue)", pointerEvents: "none", fontSize: 9 }}/>
        <i className="fas fa-chevron-down" style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", color: "var(--hl-go-blue)", pointerEvents: "none", fontSize: 8 }}/>
      </div>
    </div>
  );
}
...
function FilterChip({ tone, icon, label, active, onClick }) {
  const tones = {
    success: { bg: "rgba(65,175,75,0.12)",  fg: "#1F7A2A",          activeBg: "#1F7A2A" },
    warn:    { bg: "rgba(250,199,55,0.18)", fg: "#7A5300",          activeBg: "#7A5300" },
    info:    { bg: "rgba(5,141,233,0.12)",  fg: "var(--hl-go-blue)", activeBg: "var(--hl-go-blue)" },
    violet:  { bg: "rgba(150,95,176,0.14)", fg: "#7A4E92",          activeBg: "#7A4E92" },
    err:     { bg: "var(--hl-framing)",     fg: "var(--hl-fg-2)",    activeBg: "var(--hl-fg-2)" },
  };
  ...
  return (
    <button
      ...
      {active && <i className="fas fa-times" style={{ fontSize: 10, opacity: 0.8, marginLeft: 2 }}/>}
    </button>
  );
}
